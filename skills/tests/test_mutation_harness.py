...
REGISTRY = [
    ...
    Mutation(
        guard="test_always_read_frame",
        test="TestAlwaysReadFrameRulebook.test_names_the_section_scoped_surface",
        target=RULES,
    ...
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_an_out_of_scope_record_is_d069_operating_not_this_rule",
        target=GUARD_DOCTRINE,
        block="A record can of course be outside the\ncertified scope for D-069's own reason, and that is D-069 operating, never this\nrule.",
    ),
    Mutation(
        guard="test_fresh_context_readers",
    ...
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_replay_projection_pointer_names_its_entry",
        target=GUARD_DOCTRINE,
        block='its replay projection and tolerance are at\nD-085.',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_fourth_quantity_pointer_names_its_entry",
        target=GUARD_DOCTRINE,
        block='why the fourth is counted is at D-085.',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_supersession_argument_pointer_names_both_entries",
        target=GUARD_DOCTRINE,
        block='loosening (D-083, D-085).',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_operation_self_corrects_and_certification_does_not",
        target=GUARD_DOCTRINE,
        block='Operation\nself-corrects: suites, the mutation harness, and the sweeps all report against\nthe artifact, so an author who runs them finds its own mistakes.',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_coverage_check_forbids_a_believed_cover",
        target=GUARD_DOCTRINE,
        block='and no criterion is\n  covered only by an assert the author believes covers it.',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_accuracy_check_reads_out_of_the_file_not_the_narrative",
        target=GUARD_DOCTRINE,
        block="read out of\n  that file rather than out of the milestone's narrative.",
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_anchor_check_includes_the_targets_hard_wrap",
        target=GUARD_DOCTRINE,
        block="including under the target's hard wrap.",
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_scope_bound_leaves_the_zero_unresolved_bar_intact",
        target=GUARD_DOCTRINE,
        block='Zero unresolved stays the bar;\nwhat this excludes is a scope that regresses.',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_a_fix_authored_record_is_still_read_and_still_corrected",
        target=GUARD_DOCTRINE,
        block='A fix-authored record is still read\nand still corrected:',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_narrowing_of_the_bar_is_named_as_deliberate",
        target=GUARD_DOCTRINE,
        block="— a deliberate narrowing of the zero-unresolved bar,\nwhose cost the falsifier's clause (ii) counts.",
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_certification_does_not_self_correct",
        target=GUARD_DOCTRINE,
        block='Certification\ndoes not, because the author checks the description against its generative',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_diagnosis_names_why_the_author_cannot_see_it",
        target=GUARD_DOCTRINE,
        block='and both read as true to the person who formed the intent',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_regress_argument_names_ip4_as_its_mechanism",
        target=GUARD_DOCTRINE,
        block='that record is append-only under IP4, and so each round manufactures\nuncertified surface for the next one to audit',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_scope_bound_is_stated_as_a_necessary_condition",
        target=GUARD_DOCTRINE,
        block='Without this the gate cannot converge',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_sufficiency_arm_names_the_reader_it_protects",
        target=GUARD_DOCTRINE,
        block='and a\nreader deep in a long loop can take that silence as licence to close.',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_author_exclusion_names_the_judgment_authors_fail_at",
        target=GUARD_DOCTRINE,
        block='it asks for the judgment this section exists because authors fail\nat',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_evidence_claims_no_saving_it_does_not_produce",
        target=GUARD_DOCTRINE,
        block='and the section says so rather than claiming a saving it does not produce',
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_non_conflict_proof_states_its_own_step",
        target=GUARD_DOCTRINE,
        block="which under the\n  reopening rule means its round 1 found something, which (iii)'s zeroes forbid.",
    ),
    Mutation(
        guard="test_fresh_context_readers",
        test="TestDescriptionLayerCertification.test_the_motivating_milestone_pointer_names_its_entry",
        target=GUARD_DOCTRINE,
