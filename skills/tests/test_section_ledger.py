...
CONTRACT = TESTS / "ledgers" / "extractor-contract.md"
CONTRACT_EXPECTED = TESTS / "ledgers" / "extractor-contract.expected.txt"
...
def _module_level_assignments(source):
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            # Only a FUNCTION body leaves module scope, and only the body: a
            # decorator expression and a default argument are evaluated at
            # module scope, and round 7 bound `_TERMS` in both with the suite
            # green. A CLASS body does not leave — `class _Terms: ITEMS = [...]`
            # is reachable as a module-level constant, which is what AC1 bans;
            # methods are `FunctionDef`s and their locals are excluded here.
            for child in node.body:
                nested.update(id(n) for n in ast.walk(child))
    return [
        node for node in ast.walk(tree)
        # Round 7: the scope was derived but the binding SHAPE was still a
        # hand-picked node list, so `for`, `with ... as`, a decorator walrus
        # and a class attribute each bound the banned enumeration green. A
        # binding is a name in Store context — that is the language's own
        # definition, and it needs no list to keep extending.
...
class TestExtractorContract(unittest.TestCase):
    """The extractor's own behaviour, pinned against a fixture never regenerated.

    This closes a class rather than a case. The §8 ledger cannot pin what the
    extractor DOES: §9's remedy is to regenerate the ledger, and the comparison
    is invariant under co-regeneration — so any change to extraction behaviour
    survives if the ledger is regenerated with it. Rounds 5, 6 and 7 each found
    one instance of that (sentence fusion, then the fusion fix itself, then
    `sorted()` alphabetizing §8), and each was closed by adding one more
    hand-written fixture — an enumeration, which is the failure §3 names and
    the failure §9 ships against.

    The fixture pair below is the structural remedy: input committed, expected
    output committed, neither regenerated by this guard. One document carries
    document-order-differs-from-sorted-order, a bold headline ending `.**`,
    closing quote and parenthesis after a terminator, a code span containing a
    period, a hard wrap to normalize, a hyphenated compound, and a following
    heading to bound at. Any change to ordering, boundary handling, or
    normalization reds it.
    """

    def test_the_extractor_matches_its_committed_contract(self):
        self.assertEqual(
            sl.sentences(CONTRACT, "## 1. Zebra sorts last but comes first"),
            CONTRACT_EXPECTED.read_text().splitlines(),
        )

    def test_the_contract_fixture_is_not_regenerated_by_this_guard(self):
        # The property that makes the pair work. If any test regenerated the
        # expected file it would go co-variant with the extractor and pin
        # nothing — the exact defect it exists to close. Checked by AST and not
        # by substring: a first draft searched this file for the literal
        # `CONTRACT_EXPECTED.write_text`, which the search itself put there, so
        # the guard red on its own assertion.
        ...
        calls = {
            ...
            and node.func.value.id == "CONTRACT_EXPECTED"
        }
...
class TestExtraction(unittest.TestCase):
    ...
    def test_each_module_constant_names_the_class_it_closes_over(self):
        ...
        constants = sorted({s.lineno - 1
                            for s in _module_level_assignments(text)})
