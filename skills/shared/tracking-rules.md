...
not do is be the only place the idea exists, since nothing outside `cairn/` is
read at plan time. When a chip-creating tool fires in a cairn repo the
`idea_guard.py` PreToolUse hook injects this reminder as a non-blocking nudge —
it prompts the pairing, it does not make the call.
