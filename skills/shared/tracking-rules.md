...
order within a level; the token is the fact the order derives from, never a second encoding of it. A new row is
inserted at its level by whoever adds it, never appended below the `[low]` rows. The token is not a status, a file, or
an ID — `/cairn-triage` reads and re-rates it; a misspelled token reads as `normal` (D-134).
...
- **A branch push starts CI, tracking-only commits included.** A push of a milestone or hotfix branch starts the
  ...
  (`scripts/cairn_ci_paths.py --report`) and offers the edit under a chip where PyYAML is present
  (`--apply`), else shows the item to add by hand.
...
blocks, primary first, each with a one-line plain-language label; and one line noting that adjusting course or
`/clear` are both safe at this point. No chip is posed to route to
