...
**A universal claim over a milestone's own artifacts is a count claiming zero
...
verbatim-reproducible grade as any other count. **Where no stated procedure
can enumerate the domain, the universal is not written** — state what was
actually swept and how, and claim that instead (RR11 BC5; the M118 lesson is
the failure mode: a criterion listing its sites becomes the sweep, and every
site it omits ships stale).
