...
- Fresh-context review (three lenses, PR #170): [O] diff-bug 16 findings; [S] blame-history 4 observations (two overlapping [O]); [S] prior-review-record: no regressions of prior findings, `gh api …/pulls/comments` probe empty. Triage (7 fix-now, 0 follow-up, 13 rejected), all fix-now on `skills/milestone/SKILL.md`, suites re-run green (skills 566 by hand):
  - F1 (O1) `--limit` raise rule unsound — a truncated list can hide a long-open low-numbered PR merged after the stamp: fix-now, raise also when the returned count equals the limit.
  - F2 (O2) no rule for a stamp still reading `YYYY-MM-DD`: fix-now, a dateless stamp keeps every entry the login filter keeps.
  - F3 (O3) failure clause ambiguous over the per-PR `gh pr diff` read: fix-now, one failed diff names that PR, its item still reaches §3, the rest are kept.
  - F4 (O5) §3's route list named inbox items and orphans but not outside merges: fix-now, added.
  - F5 (O6) "four dispositions below" miscounts the five-bullet list: fix-now, "the dispositions below other than close".
  - F6 (O8) hint caveat one-sided — basename-only mentions never match a path, so "none" is not evidence: fix-now, both stated.
  - F7 (O15/S3) `/hotfix` bullet did not cover repairing what a merged PR undid: fix-now, one sentence appended after the pinned phrase (first edit broke `test_issue_triage`'s pin; reworded per lesson M148).
  - Rejected: O4/S2 no search-first sweep and same-day re-proposal (the hint is not a duplication check; re-proposal only when the stamp date equals the merge date); O7 rationale wider than the filter (the filter is stated explicitly; Out list keeps operator merges out); O9 third copy of the unreachable-`gh` rule (AC1 requires the clause stated here, and it adds "otherwise fails"); O10 null `mergedBy` (already kept — the safe direction); O11 `author` field unused (pinned by AC1; harmless in display); O12 empty archive dir grep error (edge; F3 keeps the item); O13 date derived twice from §1 output (AC1 names the ROADMAP line); O14 sentence restates its neighbour (style); O16 missing blank line before **Replace** (pre-existing on main); S4 README sentence placement (AC4 names the contributions bullet); S1 (no history conflict) is not a finding.
- Diffstat vs main after fixes: README.md +3, skills/milestone/SKILL.md +45/−4, milestone file, ROADMAP row.
