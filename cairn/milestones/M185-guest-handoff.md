...
- **Branch/PR:** m185-guest-handoff · https://github.com/jmgirard/cairn/pull/192   <!-- owner: implement (branch) / review (PR URL) · create -->
...
- [x] AC1: On a guest-mode fixture with both an `upstream` and an `origin` remote whose HEAD branches differ, `cairn_common.default_branch(root)` returns upstream's HEAD branch name and `cairn_common.base_remote(root)` returns `"upstream"`; on a guest fixture with `origin` alone, and on an owner fixture that has an `upstream` remote, `base_remote(root)` returns `"origin"` and `default_branch(root)` returns origin's HEAD branch name; every existing `default_branch` test stays green.
- [x] AC2: T2's site list is read line by line and each site either names the base remote, describes a push of the operator's own branch to the fork, or is recorded in the work log as owner-mode-only naming the skill that stops in guest mode; `grep -rni "origin" skills/*/SKILL.md skills/shared/` is then run and every hit outside that list takes the same three-way disposition; the rulebook's detection recipe names the base remote and states the guest rule (`upstream` when present).
- [x] AC3: `skills/milestone-review/SKILL.md` states the guest arm: step 1 fetches the default branch from `<base>` and never pushes it; step 2 pushes the branch to `origin` (the fork) and opens the PR with `gh pr create --repo <base> --head <fork-owner>:<branch> --draft`; step 7's chip is an approval gate with handoff recommended and decline present and no merge option; step 8 runs `gh pr ready --repo <base>` only on selection, sets `blocked`, writes the work-log line, and closes with the CI line; step 9 hygiene writes to disk and commits nothing. `skills/shared/tracking-rules.md`'s transition line adds `blocked → done` as legal for a guest-mode handoff whose PR the maintainers merged. Verified by reading each passage.
- [x] AC4: `skills/milestone/SKILL.md` §2 states the four routes for a `blocked` milestone whose header names a PR, keyed on `gh pr view <N> --repo <base> --json state,reviewDecision`: MERGED, CLOSED, OPEN with `CHANGES_REQUESTED`, OPEN otherwise; verified by reading the passage.
- [x] AC5: `skills/milestone-implement/SKILL.md` step 2 in guest mode fetches the default branch from `<base>` and never pushes it, cuts the branch from `<base>/<default-branch>`, and syncs a moved default branch by `git rebase <base>/<default-branch>`; `skills/hotfix/SKILL.md`'s merge step names the guest handoff; `skills/shared/profiles/r-package.md`'s consistency-gate names the justified NOTE; verified by reading each passage.
- [x] AC6: `CHANGELOG.md` carries an entry under the dev heading naming the base remote and the handoff; the `verify` slot's two suites exit 0 from the repo root, each exit code checked, with no new test skipped.
...
- 2026-09-10 AC1: `hooks/tests -k TestBaseRemote -k default_branch` — 8 tests green (5 new `TestBaseRemote`: guest both-remotes reads upstream HEAD and `base_remote` = `upstream`; guest origin-alone and owner-with-upstream both `origin`/origin HEAD; ls-remote fallback; no remote); the 3 pre-existing `default_branch` tests green. PASS.
- 2026-09-10 AC2: T2 work-log dispositions read against the branch — rulebook recipe, `cairn-init:83-86`, `milestone-implement:42`, `milestone-review:87` name `<base>` or the fork push; `cairn-release:37,108-109`, `cairn-triage:36`, `generic.md:46` logged owner-mode-only, and each skill's guest stop clause confirmed on the branch (`cairn-release:28-30`, `cairn-triage:30-32`). Fresh `grep -rni "origin" skills/*/SKILL.md skills/shared/` hits outside the list: `hotfix:144` (fork push), `tracking-rules:311-312` (base-remote bullet), `cairn-triage:93-95,209-216`, `milestone-plan:324`, `records-hygiene:34` (the word as provenance/"original", not a remote). Recipe at `tracking-rules:222-226` names `<base>` and the `upstream`-when-present rule. PASS.
- 2026-09-10 AC3: `milestone-review/SKILL.md` diff read — step 1 guest arm fetch-only, never pushes; step 2 fork push + `gh pr create --repo <base-repo> --head <fork-owner>:<slug> --draft`; step 7 handoff gate (recommended handoff, decline, no merge); step 8 `gh pr ready <N> --repo <base-repo>` on selection only, `blocked`, work-log line, CI line; step 9 on-disk, nothing committed. `tracking-rules.md:171-173` transition line carries `blocked → done` for a merged guest handoff. PASS.
- 2026-09-10 AC4: `milestone/SKILL.md:147-159` read — the blocked-with-PR bullet keys on `gh pr view <N> --repo <base-repo> --json state,reviewDecision` and states MERGED, CLOSED unmerged, OPEN + `CHANGES_REQUESTED`, OPEN otherwise. PASS.
- 2026-09-10 AC5: `milestone-implement/SKILL.md:38-59` read — guest arm fetch from `<base>`, never pushes, cuts `<slug>` from `<base>/<default-branch>`, syncs by `git rebase <base>/<default-branch>`; `hotfix/SKILL.md:212-225` merge-step guest handoff; `r-package.md:32-35` consistency-gate justifies the `cairn` directory NOTE by the mode. PASS.
- 2026-09-10 AC6: `CHANGELOG.md` dev heading's guest-mode entry names the base remote and the handoff gate; from the repo root `scripts/tests` exit 0, `hooks/tests` 146 tests exit 0; no `skip` in either log; branch adds 5 tests, none skipped. PASS.
- 2026-09-10 Driving RR: none — no projection pairs.
- 2026-09-10 consistency gate: `cairn_validate.py` exit 0, all checks passed (release window advisory silent); no DESIGN.md principle changed → `cairn_impact` skipped; profile `generic` names no toolchain checks. PASS.
