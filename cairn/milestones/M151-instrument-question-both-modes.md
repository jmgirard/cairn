...
- 2026-08-21: re-review fan-out triaged at the gate — repairs verified, ten fresh findings R1–R10, none floor-qualifying; R1 and R3 fixed at the gate (genus head "verifies it or records its verification"; first-pass evidence lines marked superseded), R2/R4–R10 rejected with logged reasons; explicit exits skills 0, scripts 0, hooks 0, validate 0 after the fix.
...
**Fan-out (2026-08-21, three lenses, user-facing tier).** [S] blame-history: zero defects; confirmed the change executes D-118's falsifier as pre-registered rather than undoing D-111/M145's ceremony reduction, pinned guard sentences byte-identical to main; surfaced one soft spot (the "among others" genus reading is an interpretive extension, disclosed in D-120 and backstopped by the D-108 warrant). [S] prior-PR-comments: no PR threads exist (probe empty); archived reviews M142–M150 checked; zero findings. [O] diff-bug: twelve ranked findings, recorded as F1–F12:

- F1 (most severe): the widened question and the finding/disposal rule disagree — step 3 adds "or of the record the work leaves" as a third disjunct but the next sentences stay instrument-only ("A criterion binding an instrument property is a finding… the instrument property moves…"), so a reader answering "record property" is given no rule making it a finding or routing its disposal.
- F2: D-120's stated reading (record properties fall WITHIN the instrument genus) contradicts the shipped prose, which coordinates them as a third disjunct — if the genus contained them the disjunct is unnecessary, and its presence is textual evidence against the entry's own reading.
- F3: D-120's Context misstates M101 review return 1 by omission — the M101 record shows return 1 was G7+G8 plus G1, G1 a load-bearing prose defect that is not a recording clause, so "two defect returns on recording clauses" overstates.
- F4: D-120 leaves D-111's second falsifier unnamed while its surviving ceremony falsifier now ranges over a three-question mode.
- F5: D-120's Context carries the per-return evidence account rather than leaving it in the milestone file (D-116 part 2).
- F6: "it never relaxes the probe question above, in the mode that asks it" now has a strained antecedent — a reader binding "it" to the instrument question gets a sentence attributing that question to a mode, AC1's forbidden shape.
- F7: AC4's "cited to that repo's milestone file" met only loosely — no file path in D-120.
- F8: 'the G7/G8 "unmet as written" repairs' puts quotation marks on F1-of-M101's phrasing; G7/G8 read "as written is not literally met".
- F9: review evidence and PR link were uncommitted when the reviewer ran.
- F10: CHANGELOG 1.6.0 prose ("shorter two-question form"; "The full criteria audit also now asks…") is stale as of this branch with no successor entry.
- F11: D-118 part 2 affirms D-111's "reduced two-question form… stand[s]"; D-120 supersedes that form but does not name this D-118 clause.
...
**Re-review fan-out (2026-08-21, three fresh lenses).** [S] blame-history: clean — D-120 byte-unchanged across the branch, pinned sentences intact, D-121's form matches the D-096/D-100 precedent. [S] prior-PR: zero findings — F1/F2/F6 verified cured, no F1–F12 reintroduced, scope-out boundary holds. [O] diff-bug: verified all six actioned repairs (quotations character-for-character against D-118/D-111/D-120 and the M101 record; D-120 pure-append; suites independently re-run 0/0/0; "AC2 is literally met") and returned ten fresh findings, R1–R10. Triage at the gate — none crosses the return floor (no AC fails on its plain reading; the floor's load-bearing arm excludes doctrine prose about how work is verified). Fixed now: R1 (the genus head "an instrument that verifies it" ill-described the record exemplars inside its list — genus now reads "an instrument that verifies it or records its verification", so the instrument-property sentences reach the record exemplars semantically, with no disjunct for F2 to re-fire on); R3 (the two first-pass AC evidence lines now marked superseded-at-return-1, so the archived record cannot be read against stale text). Rejected with reasons: R2 (the plain reading attaches "in the full mode that asks it" to the probe question; F6 was actioned once — further churn on conceivable misparses is the spiral this milestone exists to stop); R4 (the heading's "an unnamed D-118 clause the supersession reaches" names item 3's subject; "Also recorded" is disclosure, not a fourth correction; D-096's claims-corrected idiom is the precedent); R5 (the claim is supported by the M101 record the entry cites — M101's G7/G8 triage lines each read "fix now, and a reason for the return" — merely uncited in-line; a D-122 to add a cite is the correction-chain shape); R6/R9/R10 (style observations; R10 cosmetic by its own grading — a recording clause narrows to the deliverable property its record was about); R7 (a correction entry must quote what it corrects — every batched-correction precedent does; F5's rejection rationale stands); R8 (F10's rejection stands — a versioned changelog section describes its release; the 1.7.0 consolidation describes this change).
