...
- Fan-out (three lenses + scorer): blame-history — no findings (D-057 carve-out exempts these mechanisms; chip change doesn't touch D-019; absent Driving RR slot on M100's own header mirrors M38's grandfathering). Prior-PR — no prior-PR evidence (zero review comments/reviews across all 98 PRs; 16th recorded no-op, further evidence for M101's repoint). Diff-bug — 4 findings, all fail-open paths in check_binding_criteria.
- Findings triage: F1 (92, fixed) deviations/verbatim text inside HTML comments counted though never shown — comments now stripped (`_HTML_COMMENT_ML`) before both the corpus and the deviations scan. F2 (92, fixed) present-but-unparseable Binding criteria section bound nothing silently — now fails loud ("no parseable '- BC<n>:' item"). F3 (85, fixed) unbounded marker match let "Deviations from RR50" excuse an RR5-driven milestone — marker now word-bounded. F4 (78, sub-threshold, logged AND fixed — same fail-loud cluster as F2, near-free) malformed slot (`RR05,`) silently skipped the milestone — now a finding. 6 regression tests added (TestFailLoudAndUnslippable); 5 proven to fail against the pre-fix check, the sixth is the positive twin. Post-fix: 19/19 check tests, all three suites green (S:0 SC:0 H:0), validate exit 0.
