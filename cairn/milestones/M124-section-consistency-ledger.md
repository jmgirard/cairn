...
- [x] T8 — Full verify and `cairn_validate`; then `/milestone-implement` step 8
...
- 2026-07-30: §8 CERTIFICATION ROUND 5 — 2 reopening, 0 fix-authored, 0 out-of-mandate. Both fixed, both reproduced first. Round 5 found the deepest defect of the certification and it was in the instrument itself.
- 2026-07-30: round 5's F2 — the extraction was NOT returning sentences. `_SENTENCE_BOUNDARY` required whitespace IMMEDIATELY after the terminator, and §8's rule headlines end `.**`, so six ledger units each fused a bold headline with the sentence after it (verified independently before acting: lines 1, 4, 25, 34, 38, 41; line 25 was "**A round reopens only on a finding within the three named checks above.** Those three are the whole of this step's mandate."). Four rounds passed over it because the extraction's own tests use toy fixtures with no markdown emphasis. The boundary now takes terminal punctuation, then a run of closing markup, then whitespace, and the split moved to `finditer` so the terminator and its markup stay with the sentence they end — `re.split` would have dropped the `**`, and a ledger that silently discards emphasis cannot see a rule lose its bold.
- 2026-07-30: CORRECTION, appended not edited (IP4) — every "50 sentences" and "51 units with the heading, 50 without" in this log and in the test comments described the FUSED extraction. At HEAD the ledger is 56 units, 57 with the heading. The T1 line's "no suspicious units" was true only of the check it ran, which looked for heading fragments and short strings, not for fusion.
- 2026-07-30: round 5's F1 — both AC1 scans iterated `tree.body` alone, so a constant one block deep was invisible: `if True:` + `_TERMS = [...]` shipped green, the exact enumeration AC1 bans. Both now share `_module_level_assignments`, which descends into block statements and stops at `FunctionDef`/`ClassDef` — keeping round 3's O2 function-local case correctly out of scope, since AC1 names a parameter or a module constant and a local is neither.
- 2026-07-30: round 5 verify — 3/3 defeats reproduced then RED; AC3's load-bearing replay re-run against the regenerated 56-unit ledger, (a) and (b) still red the ledger guard and NOTHING else; skills 808 / scripts 332 / hooks 103 each exit 0; `cairn_validate` exit 0.
- 2026-07-30: §8 CERTIFICATION ROUND 6 — 2 reopening, 3 fix-authored, 1 out-of-mandate. All fixed.
- 2026-07-30: round 6's F1 is the sharpest finding of the certification: round 5 fixed the fusion INSTANCE and added no assert, so reverting `_SENTENCE_BOUNDARY` and regenerating the ledger shipped the defect again with 808 green. The ledger guard structurally cannot catch it — regeneration is exactly what §9 prescribes as the remedy, and the comparison is invariant under co-regeneration. Every other extraction fixture is markup-free, which is why five rounds passed over it. Now pinned on a markup-bearing fixture, and verified through the whole regeneration path: revert the boundary, regenerate to 50 units, suite FAILS.
- 2026-07-30: round 6's F2 — round 5's `_module_level_assignments` replaced one hand-picked traversal with a hand-picked list of four block fields, and `match`/`case` bodies and a module-scope walrus both still shipped `_TERMS = [...]` green. Extending the field list a third time is the fifth-extension move this milestone exists to stop, so scope is DERIVED now: everything under a function or class goes into an exclusion set, and every binding outside it is module scope. No new block syntax can open a hole, and function-locals stay correctly out.
- 2026-07-30: round 6's fix-authored records corrected in place: `moved=23` is stale since round 5's regeneration — at 56 units mutation (a) inserts at index 30, so index comparison gives `moved=26` (both sites); `_module_level_assignments`'s "however deeply nested in blocks" was false of the field-list version; and the module docstring said ONE closed punctuation class 27 lines above a comment saying TWO — §9's own contradicting-sentence shape inside the module shipping the guard against it, now one carve-out constant closing over two classes.
- 2026-07-30: CORRECTION, appended not edited (IP4) — the round-2 and round-5 correction lines say "insertion index 27" and `moved=23`. Both described the fused 50-unit extraction. At the shipped 56-unit ledger mutation (a) inserts at index 30 and index comparison gives `added=1, removed=0, moved=26`.
- 2026-07-30: round 6's O1 fixed: the comment scan required `startswith("#")` on the raw line, so an INDENTED comment — the shape a nested constant carries, which is exactly what round 5 taught the scan to find — would have red the guard on a correctly-commented constant.
- 2026-07-30: §8 CERTIFICATION ROUND 7 — 2 reopening, 1 out-of-mandate. Its F2 was round 6's F1 on a different property: replacing the extractor's `return [...]` with `return sorted(...)`, then regenerating the ledger, alphabetized §8 with 809 tests green.
- 2026-07-30: USER OVERRIDE, logged not resisted (tracking-rules) — the maintainer stopped the round loop at 7 rather than running to zero unresolved, after two hours and a correct diagnosis: every round's fix had been one more enumeration, which is the failure §9 itself ships against. §8's gate says zero unresolved; this milestone enters review at a stated deviation instead, and the three-lens fan-out reads the diff independently before anything merges.
- 2026-07-30: the structural remedy the override bought, which no further round would have produced on its own. The §8 ledger CANNOT pin the extractor: §9's remedy is to regenerate it, so the comparison is invariant under co-regeneration and every extractor property needed its own hand-written fixture — an infinite well, and rounds 5, 6 and 7 each drew one bucket from it. `ledgers/extractor-contract.md` plus `.expected.txt` is a ledger for the EXTRACTOR, committed on both sides and never regenerated by the guard. Verified WITH the §8 ledger co-regenerated: `sorted()` output, a boundary refusing markup, normalization dropped, and a bound running past the next heading each RED. The last two were never found by any round.
- 2026-07-30: round 7's F1 closed the same way — round 6 derived the SCOPE but left the binding SHAPE a hand-picked node list, so `for`, `with ... as`, a default-arg walrus, a decorator walrus and a class attribute each bound the banned enumeration green. A binding is now a Name in Store context, which is the language's own definition and extends to no new syntax; only FUNCTION bodies leave module scope, so a class attribute is caught and a function-local correctly is not. 6/6 shapes red, function-local green.
- 2026-07-30: `test_the_contract_fixture_is_not_regenerated_by_this_guard` red on its own assertion string at first — it searched this file for a literal the search itself put there. Checked by AST now. The fixed-point shape M99's lesson names, in a guard about guards.
- 2026-07-30: T8 verify — skills 811 / scripts 332 / hooks 103, each exit 0; `cairn_validate` exit 0. Status -> review.
- 2026-07-30: KNOWN RESIDUE at the review gate, disclosed rather than left implicit. Round 7's O1: the markup fixture exercises `**` and not other closing delimiters — inert on §8, whose 56 boundaries are only `.** ` and `. `, and the contract fixture now covers quote and paren. Rounds 3-4's O1/O2/O3 and round 4's reflow false-reds are routed to the rollout candidate row, whose promotion condition they meet as written. And no round after 7 was run, so the usual zero-unresolved evidence is absent by decision, not by oversight.
