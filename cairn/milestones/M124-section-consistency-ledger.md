...
- [ ] AC4 — Re-wrapping every paragraph of §8 to a different column width, with
      no word changed, leaves the AC2 guard green; the evidence command is
...
- [ ] T1 — Author the section-consistency helper: heading-delimited slice
      ...
      (AC1).
- [ ] T2 — Generate the §8 ledger fixture from the helper and commit it; author
      the guard test comparing section to ledger, with an added/removed/moved
      failure message (AC2).
- [ ] T3 — Register the ledger guard in `test_mutation_harness.py`; confirm it
      fails when blanked, and that `TestRegistryCompleteness` stays green
      (AC2, AC6).
- [ ] T4 — Replay the four recorded mutations against the AC2 guard, restoring
...
- [ ] T5 — Reflow §8 to a different column width, run the AC2 guard scoped,
      restore byte-identical; record (AC4).
- [ ] T6 — Append §9 to `guard-doctrine.md` — presence-vs-consistency, the two
      defeating shapes, the detect-never-judge boundary. No renumbering (AC5).
