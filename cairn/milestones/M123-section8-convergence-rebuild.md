...
- [x] T16 — Section-wide inversion sweep, then certification rounds under the
      rebuilt rules until the gate opens; suites and `cairn_validate` (AC8, AC9, AC13).
...
- 2026-07-30: §8 CERTIFICATION ROUND 4 (fourth fresh-context [O], authored none of this; run under the rebuilt rules): 8 findings — 5 REOPENING (F1-F5), 1 FIX-AUTHORED RECORD (F6), 2 OUT-OF-MANDATE (F7 seven surviving mutations, F8 the banked contradiction class). **The gate does not open.** Round counts across this certification: 16, 13, 10, then 5.
- 2026-07-30: round 4's F2, the finding that matters most — return 1's A3 fix went to the COPY. Clause (iii) gained its round scope and window scope at T13; clause (i), which is the clause A3's own comment records the defect as having been FOUND in, kept an anchor opening after both, so "in any single milestone" and "including each milestone's first" each inverted green. Fixing the instance and not the class, inside the milestone that ships a rule about exactly that.
