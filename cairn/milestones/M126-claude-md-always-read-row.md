...
- 2026-07-31: review round 2's A16 (80) fixed — the candidate row added on this branch ended mid-sentence at "corroborated by M126's own stray"; completed to name the stray work-log line inside `## Acceptance criteria` and that a certification round, not the author, found it. A17 (58) fixed in the same edit: the promotion condition read "lands in a section it was not aimed at again", a second-occurrence count, and now states the falsifier of "the author notices", firing on the first instance.
- 2026-07-31: review round 2's A5 (85) fixed — the position guard ordered by string index, so the audit-paragraph upper bound travelled with its subject. It now checks BLOCKS within the `## Always-read governance` section: the statement must be its own paragraph, after the table and after the fifth-surface paragraph, before the audit paragraph. Probes under `python3 -m unittest discover -s skills/tests`: statement alone to EOF RED; statement and audit paragraph moved to EOF together RED; statement above the fifth-surface paragraph RED; statement spliced into the middle of that paragraph RED; unmodified control GREEN.
- 2026-07-31: review round 2's A3 (83) is void rather than fixed — its subject, "the guard's own derivation", was `TestAnchorDescriptionMatchesTheAnchors`, deleted at T7. The round-2 AC4 evidence line resting on it describes nothing that exists; the Review section is review-owned, so the next round records fresh AC4 evidence rather than this session editing it.
- 2026-07-31: status -> review. Three suites green (103 / 332 / 827) and `cairn_validate .` exit 0.
