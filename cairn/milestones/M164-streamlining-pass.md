...
- 2026-08-29 gate triage (maintainer chose "fix all five" + merge approval). Fix-now: F1 — comment at `_base_commit` naming the 10s-timeout silent fallthrough (also the blame-history lens's one item); F2 — two TestNext tests pinning the `review` and plan-fallback recommendation lines; F3 — `TestMissingRRFileFailsLoud` firing the missing-RR-file branch with its message asserted; F6 — stray blank line removed; F7 — unused `import re` removed. Rejected: F4 (the guarding comment exists and the repo ships no import-sorting tooling that would reorder), F5 (the comment was RR14's own Beyond-the-brief proposal — an intentional, in-spec choice), F8 (outside R12's stated scope — test-side call sites were not part of the applied recommendation). Discrimination of the three new tests proven by scratch-copy mutation: each reds on its planted message change (identity asserted in the failure diff) and greens on the unmutated control. Suites after fixes: scripts 326 OK exit 0 (323 + 3 new tests), hooks 121 OK exit 0 — AC2's accounting becomes 327 − 4 applied removals + 3 gate-directed additions.
