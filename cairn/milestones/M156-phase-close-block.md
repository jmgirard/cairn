...
- AC1 — PASS. tracking-rules.md 260-267: "Every phase or skill ends with a **close block**, never a chip. The turn's final rendered text carries: an outcome recap ... a status table or line ... the next command or commands in fenced blocks, primary first, each with a one-line plain-language label; and one line noting that adjusting course or `/clear` are both safe at this point. No chip is posed to route to the next skill — the user runs the fenced command"; decision gates spared at 265-266 ("Decision-gate chips, the merge-approval gate among them, are unaffected: a gate is a choice, a phase end is a handoff"); the review-end exception is rewritten at 266-267 ("generalized from what was once review's sole exception").
- AC2 — PASS. The three bullets on the branch: Mandated substance renders 298-304 ("A decision chip is posed in the same turn as its presentation, and its question text and option descriptions alone carry what the choice needs, in plain language; fuller evidence appears best-effort in the chat above, and where it exists as a file on disk ... the question text cites its path"); Chips carry choices 305-310 ("never a reason to defer the chip to another turn"); Acceptance chips 322-328 ("the chip is posed in the same turn as its presentation"). No deferral clause remains; corroborating grep `posed after the user responds` over skills/*.md returns nothing (exit 1).
