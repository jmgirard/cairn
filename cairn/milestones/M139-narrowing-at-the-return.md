...
- 2026-08-14: RR12 ingested (the AC4 guard-instrument escalation, RB12). Answers: (Q1) the three AC4 failures are one invariant violated alternately — totality (pinned extent = slice) and granularity (slice = one rule); a fragment-anchor family always leaves an unpinned complement, and the complement carried each next inversion. (Q2) a regex family delivers AC4's property only in the degenerate limit of one whole-slice equality check per rule — every design short of that re-introduces hand enumeration. (Q3) recommended instrument: normalized whole-slice `assertEqual` against verbatim in-test fixtures; content-hash, AST-parse and probe-generator forms rejected. (Q4) disposition: split — ship the doctrine on AC1–AC3/AC5/AC6, prune AC4 via the gated amendment (touching AC5's cross-reference and the two-clauses→three-limbs wording, R6/FE), child milestone carries BC1–BC7 verbatim. (Q5) corpus-wide remedy refused at D-090's unsatisfied shipped-behavior trigger; guard-doctrine banking (the two-invariant statement, the insertion axis) is the child's ordinary work. Promoted to D-103.
- 2026-08-14: RR12 recommendation triage. Apply: rec 1 (split routing — the gated amendment is the next act on this branch), rec 2 (equality instrument), rec 3 (implement-side sub-slice), rec 4 (one-time insertion-gap probe at the child's review) — recs 2–4 scheduled into the child via BC1–BC7. Consider, adopted: rec 5 (guard-doctrine banking, child's ordinary work), rec 6 (dated note on the ROADMAP one-surface-pin row applied at this ingest; B4's registry-comment fix left to the child's registry rewrite). Recs 7–10 are RR12's own rejections, adopted with its reasons (hash form; AST parse; standing probe generator; corpus-wide sweep).
