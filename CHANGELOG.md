...
- **`/cairn-init` says that tracking-only pushes start CI, and adds the
  ...
  flow-list `on:`, a flow-mapping `push`, a `push` with `paths`, a
  flow-sequence `paths-ignore`, one already ignoring `cairn/**`, more than
  one document, or a file PyYAML cannot parse. A refused file, and every
